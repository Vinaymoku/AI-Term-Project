class Game():
    ...
    def qLearning(self, board, QKey):
        """If we have two in a row, column, or diagonal and the 3rd is available, take it. """
        for i in range(3):
            # Check rows
            row = self.boardValues[i * 3 : (i * 3) + 3]
            if row.count(" ") == 1 and row.count(QKey) == 2:
                idx = row.index(" ")
                return (i * 3) + idx

            # Check columns
            col = self.boardValues[i : 9 : 3]
            if col.count(" ") == 1 and col.count(QKey) == 2:
                idx = col.index(" ")
                return i + (idx * 3)

        # Check diagonals
        diag_left = [self.boardValues[0], self.boardValues[4], self.boardValues[8]]
        diag_right = [self.boardValues[2], self.boardValues[4], self.boardValues[6]]

        if diag_left.count(" ") == 1 and diag_left.count(QKey) == 2:
            idx = diag_left.index(" ")
            return idx * 4

        if diag_right.count(" ") == 1 and diag_right.count(QKey) == 2:
            idx = diag_right.index(" ")
            return 2 + idx * 2
